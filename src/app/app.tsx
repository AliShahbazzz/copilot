...
const loadInitialConfig = (): CopilotAuthConfig | null => {
  ...
  if (rawSellerDetails) {
    try {
      sellerDetails = JSON.parse(
        rawSellerDetails,
      ) as CopilotAuthConfig["sellerDetails"];
    } catch {
        ...
        <Thread
          welcome={{
            suggestions: [
              ...
              {
                text: "Show customers from Mumbai",
                prompt: "Show me all customers from Mumbai",
              },
              {
                text: "Payment reminders",
                prompt: "Show me all pending customer enquiries",
              },
              {
                text: "Show recent unread messages",
                prompt: "Show me all unread messages in last 24hrs",
              },
            ],
          }}
        />
...
export default function App() {
  const [config, setConfig] = useState<CopilotAuthConfig | null>(() =>
    loadInitialConfig(),
  );
            ...
            <div className="flex h-full items-center justify-center">
              <div className="rounded-lg border border-slate-200 bg-white p-6 text-center shadow-sm">
                ...
                <p className="mt-2 text-xs text-slate-600">
                  Use MiniLogin on the left to fetch seller token and workspace
                  context.
                </p>
              </div>
            </div>
